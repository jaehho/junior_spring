...
[pks, locs] = findpeaks(20*log10(avg_A_scan_M1), 'MinPeakHeight', -5);
...
% Displacement (in nm) = (center_wavelength/(4*pi)) * unwrapped_phase * 1e9.
pixel_index = 1085;

%% (a) SDPM on the 1-tone MScan
% Extract the time series at the selected pixel
time_series_M1 = A_scans_M1(pixel_index, :);

% Unwrap the phase to avoid
phase_unwrapped_M1 = unwrap(angle(time_series_M1));
figure;
plot(phase_unwrapped_M1);
title('Unwrapped Phase (MScan1)');
keyboard;
...
% Plot the time-domain displacement
figure;
plot(t_M1, displacement_M1);
title('Displacement vs. Time (MScan1)');
xlabel('Time (s)');
ylabel('Displacement (nm)');
saveas(gcf, 'figures/Displacement_Time_MScan1.svg');

% Frequency domain analysis: compute FFT and frequency axis
...
xlabel('Frequency (Hz)');
ylabel('Amplitude');
saveas(gcf, 'figures/Displacement_Freq_MScan1.svg');

%% (b) SDPM on the 40-tone MScan
A_scans_M40 = BScan_M40(:, D_MScan+1:end);   % extract A-scans (complex data) from MScan40
time_series_M40 = A_scans_M40(pixel_index, :);
