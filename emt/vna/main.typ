...
#show: ieee.with(
  ...
  abstract: [This report documents the use of the Agilent E5071C Vector Network Analyzer (VNA) to characterize the performance of a Mini-Circuits ZAPDQ-2-S power splitter over the 0.8–2.2 GHz frequency range. Utilizing the 85052D calibration kit, S-parameters were measured at five distinct frequencies (0.85, 1.15, 1.5, 1.85, and 2.15 GHz) to assess insertion loss, return loss, phase imbalance, amplitude imbalance, isolation, and VSWR. The results are compared with theoretical and specified values to evaluate the device's compliance with nominal performance criteria. Key deviations were observed, particularly in phase imbalance, while amplitude and isolation generally aligned with expectations.],
  authors: (
    (
      name: "Noam Schuck",
      department: [ECE335],
...
The primary objectives of this experiment were to develop proficiency in using the VNA and to compare theoretical circuit behavior to empirical RF measurements.
...
The average deviation of the amplitude of the s-parameters is shown in @tab:deviation-log-magnitude. The deviation is calculated by taking the difference between the experimental and theoretical values of the log magnitude of the s-parameters. The deviation is within 1.5 dB for all frequencies except for 1.15 GHz, where the deviation is 2.415 dB.

@tab:supp_measurements shows the amplitude imbalance, phase imbalance, and isolation of the splitter. The experimental amplitude imbalance appears relatively close enough with the theoretical values; the phase imbalance on the other hand is not. The isolation also appears to match the specs as the signal is still attenuated significantly in the experimental results.
...
#{
  ...
  [#figure(
    ...
    grid(
      ...
      table(
        table.cell(colspan: 4, [Theoretical]),
        [Frequency [GHz]], [Amplitude\ Imbalance [dB]], [Phase\ Imbalance [dB]],[Isolation [dB]],
...
#figure(
  caption: [Experimental VSWR Values],
  placement: auto,
  table(
    columns: (1in, 1fr, 1fr, 1fr),
    fill: (x, y) => if x == 0 or y == 0 { silver },
